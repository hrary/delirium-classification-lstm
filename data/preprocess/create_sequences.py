...
def create_sequences(df, resolved_cols, seq_length, step):
    ...
    # compute long-term HR average and insert deviation column after HR
    ...
    # add HRV placeholder and enforce final column order
    if "HRV" not in df_sub.columns:
        df_sub["HRV"] = np.nan
    # restrict FEATURE_ORDER to columns we have (HRV present), then reindex
    ...
    for start in range(0, len(df_sub) - seq_length + 1, step):
        ...
        # write the scalar HRV into the HRV column for all timesteps in the window
